...
int minmax(Plateau* p, int player, int iaPlayer, int deep, int niveau, int a, int b) {
	int rival = player == 1 ? 2 : 1;
	if (deep > 0) {
		int m = player == iaPlayer ? INT_MIN : INT_MAX;
		int c = 0;
		for (Stack* s = get_all_moves(p); s != NULL; s = stack_pop(s)) {
			p->tab[s->val] = player;
			int tmp = minmax(p, rival, iaPlayer, deep - 1, niveau, a, b);
			if (player == iaPlayer) {
				if (tmp > m) {
					m = tmp;
					c = s->val;
		...
		return evaluation(p, player, niveau);
}
...
*/
int ia(Plateau* p, int player, int deep, int niveau) {
	int coup = minmax(p, player, player, deep, niveau, INT_MIN, INT_MAX);
	return coup;
}
...
int main() {
	...
	if (true) {
		int coup = ia(p, rival,deep, niveau);
		p->tab[coup] = rival;
		printf("coup : %c\n", coup + 'a');
	...
	while (false) {
		...
		// 1
		printf("player : 1\n");
		int coup = ia(p, player, deep, niveau);
		if (coup == -1)
			break;
		p->tab[coup] = player;
		printf("coup : %c\n", coup + 'a');
